...
export class FormRendererMainComponent implements OnInit, DoCheck {
    ...
    jumpTo(event: any) {
        ...
        if (!this.isReadOnly) {
            ...
            // on submit
            if (isSubmit) {
                this.currentFormIndex = selectedIndex;
                this.onSave();
                this.stepper.reset();
                return;
            }
        ...
        formArr.forEach(element => {
            ...
            if ((config.maximum || config.minimum) && element.formBuilderComponent.isVisible()) {
                ...
                if (+elemValue > +config.maximum || +elemValue < +config.minimum) {
                    isValidAll = false;
                    elementError = true;
                    element.formBuilderComponent.setError(`Value should be inbetween ${config.minimum} - ${config.maximum}`)
                }
            }
            ...
            if (config.charachterLength && element.formBuilderComponent.isVisible()) {
                ...
                if (elemValue.length > config.charachterLength) {
                    isValidAll = false;
                    elementError = true;
                    element.formBuilderComponent.setError(`Max char limit is ${config.charachterLength}`)
                }
            }
            ...
            if (!element.formBuilderComponent.isValid() && element.formBuilderComponent.isVisible()) {
                isValidAll = false;
                elementError = true;
                element.formBuilderComponent.setError(`INVALID: Please provide valid details`)
            }
            ...
            if ((config.required == true || config.required == 'true') && element.formBuilderComponent.isVisible()) {
                ...
                if (elemValue == undefined || !elemValue) {
                    isValidAll = false;
                    elementError = true;
                    element.formBuilderComponent.setError(`Required`)
                }
            }
    ...
    validateExpressions(expressions: any[], docId: any = null, value: any = null) {
        ...
        expressions.forEach(expressionItem => {
            if (!isArray(expressionItem)) {
                console.log('Old expression array pass')
                return;
            }
            let expressionSubString = '';
            expressionItem.forEach((expression: ExpressionModel) => {
        ...
        console.log('expressionString', expressionString, eval(expressionString));
